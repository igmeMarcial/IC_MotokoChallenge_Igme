...
actor class StudentWall() {
  ...
  public shared query func getMessage(messageId : Nat) : async Result.Result<Message, Text> {
    if (messageId >= wall.size()) {
      return #err("Mensaje inválido1");
    } else {
      let messageOpt = wall.get(messageId);
      switch (messageOpt) {
        case (null) {
          return #err("Mensaje no encontrado");
        };
        case (?message) {
          return #ok(message);
        };
      ...
      switch (messageOpt) {
        ...
        case (?message) {
          if (caller != message.creator) {
            return #err("No tienes permiso para actualizar este mensaje");
          } else {
            let updatedMessage = {
              vote = message.vote;
              content = c;
